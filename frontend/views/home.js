function initHomePage() {
  ...
  if (currentHash === '#login' || currentHash === '#register') {
    console.log("initHomePage: BLOCKED - on auth page:", currentHash);
    return; 
  }
  ...
  PetService.getAllPets(function(pets) {
    if (Array.isArray(pets) && pets.length > 0) {
      renderFeaturedPets(pets.slice(0, 4));
      updateStats(pets);
    }
  }, function(xhr) {
...
function updateStats(pets) {
  const availableCount = pets.filter(p => p.availability === 'available').length;
  $('.stat-card h3').eq(3).text(availableCount);
}
